...
import { 
  ...
  CheckCircle2, Loader2, Heart} from 'lucide-react';
  ...
  useEffect(() => {
    async function fetchAllData() {
      try {
        ...
        if (settingsRes.data && settingsRes.data.hero_images && settingsRes.data.hero_images.length > 0) {
...
<section className="relative min-h-[90vh] md:min-h-screen flex flex-col justify-center bg-[#0A192F] pt-20">
  ...
  <h1 className="text-slate-500 text-[20px] md:text-[20px] font-bold  tracking-widest">
  ...
  <div className="absolute bottom-0 right-0 z-20 bg-white p-6 md:p-12 grid grid-cols-3 md:flex gap-6 md:gap-12 shadow-2xl">
    <div>
      ...
      <div className="text-slate-500 text-[8px] md:text-[10px] font-bold  tracking-widest">Years Exoerience</div>
    </div>
    <div>
      <div className="text-2xl md:text-5xl font-black text-[#1E40AF]">100+</div>
      <div className="text-slate-500 text-[8px] md:text-[10px] font-bold  tracking-widest">Projects Done</div>
    </div>
    <div>
      <div className="text-2xl md:text-5xl font-black text-[#1E40AF]">90+</div>
      <div className="text-slate-500 text-[8px] md:text-[10px] font-bold  tracking-widest">Happy Clients</div>
    </div>
  </div>
</section>
...
<section className="py-20 md:py-32 bg-white">
  <div className="max-w-7xl mx-auto px-6">
    <div className="text-center mb-16">
      <h3 className="text-[10px] md:text-xs font-black tracking-[0.4em] text-[#15803D] mb-3">
        Our Commitment
      </h3>
      <h2 className="text-4xl md:text-6xl font-black text-[#1E40AF]  tracking-tighter">
    ...
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 md:gap-0">
      {[
        ...
        >
          ...
          <h4 className="font-black text-sm  tracking-widest text-black mb-2">
            {item.title}
          </h4>
          <p className="text-[10px] text-slate-500 leading-relaxed font-medium  tracking-tight">
            {item.desc}
          </p>
      ...
      <section className="py-24 bg-white border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h3 className="text-[10px] md:text-xs font-black tracking-[0.4em] text-[#15803D] mb-3">Professional Network</h3>
            <h2 className="text-3xl md:text-5xl font-black text-[#1E40AF] tracking-tighter ">Our Clients & <span className="text-[#15803D]">Partners</span></h2>
          </div>
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex justify-between items-end mb-16">
            <h2 className="text-4xl lg:text-6xl font-black text-[#1E40AF]  tracking-tighter">Latest <span className="text-[#15803D]">Projects</span></h2>
            <Link href="/projects" className="px-8 py-4 bg-white border-2 border-[#1E40AF] text-[#1E40AF] font-black  text-xs hover:bg-[#1E40AF] hover:text-white transition-all flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {latestProjects.map((project) => (
              <div key={project.id} className="group bg-white rounded-[32px] overflow-hidden shadow-lg hover:shadow-2xl transition-all">
                ...
                <div className="p-10">
                  <h3 className="text-2xl md:text-3xl font-black text-[#1E40AF] tracking-tighter mb-4 group-hover:text-[#15803D] transition-colors">{project.title}</h3>
                  <Link href={`/projects/${project.id}`} className="inline-flex items-center gap-2 text-[#1E40AF] font-black  text-[10px] tracking-widest">
